...
| **S17** | Front Matter: 标题、摘要与前置信息 | 结构化摘要数值与 Results 保持绝对一致，作者与单位信息完整 |
| **S18** | Journal Selection: 目标期刊选择与指南抓取 | 基于接受概率的期刊筛选矩阵，真实抓取并快照作者投稿须知（Author Guidelines） |
| **S19** | Language Polish: 事实保护性语言润色 | 执行快照 Diff 比对，**严禁润色篡改任何数值、引文或图表编号**，去除 AI 味 |
| **S20** | Submission Package: 完整投稿包封装 | 生成对照期刊指南清单的完整投稿包，零冗余孤儿文件 |
